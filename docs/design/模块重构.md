...
## 附录 A：2026 现状核对与补充（后续评审阶段增补）

原文写作时侧重"改造目标"，但近期 CCU V2 (parallelDim 修复)、size_boundary 32KB/48KB 档位等工作已经显著改变了起点。本附录对齐到 2026 当前主线状态，补上原文漏掉的三点。

### A.1 已存在的两级 dispatch（不是单层）

计划原文把决策简化为"HCCL vs CCU"，但 main 分支实际结构是：

第一级在 `ops/allgather_batch/src/all_gather_batch.cc`：`CUSTOM_COMM_USE_CCU` env var 决定走 `decomposed::Dispatch` 还是 `ccu::Dispatch`；

第二级在 `ops/allgather_batch/src/ccu_dispatch.cc`：`enum class CcuMode { kSched, kMs }` 由 `ParseCcuModeEnv()` 一次解析后缓存，再根据此决定调 `ccu_sched/` 还是 `ccu_ms/` 下的实现；

第三层曾经在 `ccu_ms/` 内部保留 V1 / V2 两份 kernel，由 `CUSTOM_COMM_CCU_MS_IMPL` 环境变量切换；现已合并为单一 `ccu_kernel_ag_batch_mesh1d_ms.cc`（基于 `ccu_base::CcuKernelAlgBase::GroupBroadcastBatch`），env 切换机制已移除。

Phase 2 重构的 Registry 需要反映两层嵌套：Backend 家族 (decomposed/ccu) 和 Variant (sched/ms)；不要把两层压平到同一个 registry key。

### A.2 `ops/op_common/` 已经存在，与计划的 `_core/` 冲突

提案写的是 `ops/_core/`，但代码仓库里已有 `ops/op_common/`（承载 CCU V2 需要的 `CalGoSize` 基类 `ccu_kernel_alg_base`）。现有的 `op_common` 是算子间共享的 SDK 扩展点（基类 + 算法工具），而计划里的 `_core/` 偏向"跨 op 的 shim-layer 基础设施"（registry、dtype mapper、error model）。两者职责不同，建议：

- 保留 `ops/op_common/` 作为"跨 op 的 CCU-kernel-layer 复用代码"容器（现状就是这个角色）
- 新建 `ops/_core/` 存放"跨 op 的 C-ABI shim-layer 基础设施"（registry、dtype converter、opaque handle 工厂）

两者物理上平行，职责上正交。命名上 `_core/` 以 underscore 打头强调它是 internal（类似 CPython 的 `_ctypes`）。

看源码 `ops/allgather_batch/src/ccu_ms/`：

- `ccu_kernel_ag_batch_mesh1d_ms.cc`（唯一实现）

- 核心抽象：`comms/torchcomms/TorchCommBackend.hpp:32-80` — `TorchCommBackend` 纯虚基类
- Registry：`comms/torchcomms/TorchCommFactory.hpp` + `.cpp` — `register_backend(name, factory)` + `create_backend(name)` 的全局表
- 每个 backend 自注册：`comms/torchcomms/nccl/TorchCommNCCL.cpp` 底部的 `static auto _register = ...::register_backend(...)` 模式
- ABI 边界：C++ 内部，但 NCCL/Gloo 底层通过 `dlopen` 动态加载，避免把具体 backend 符号拖入共享库

### B.2 TransformerEngine（C-ABI + framework adapter）

- C 头文件集中在 `transformer_engine/common/include/transformer_engine/transformer_engine.h` — 纯 C + opaque 指针
- C++ 实现在 `common/*.cu` / `common/*.cpp`，编译成 `libtransformer_engine.so`
- PyTorch 绑定在独立的 `pytorch/csrc/extensions.cpp`（以及按 feature 切分的 `extensions/activation.cpp` 等），只引用 C 头文件
- 启示：ABI firewall 不止是"不跨 std:: 类型"，还包括"每个 feature 一个 adapter 文件，避免把所有 binding 堆在一处"

### B.3 omni-ops（`/Users/shanshan/repo/vllm-project/omniai/omni-ops/`）

- 每个 op 一个独立目录 `omni_ops/<op_name>/`，内部分 `op_host/`（host dispatcher）、`op_kernel/`（device kernel）、测试用例
- 每个 op 有自己的 `CMakeLists.txt`，顶层用 `add_subdirectory` 枚举即可扩展
- 启示：算子粒度的模块化对 Phase 3（每新增一个 op 的机械步骤最少化）是关键参照

### B.4 设计原则文档

原则锚点集中在 `~/repo/_me/skills/vibe-cod/references/design-code-principles/software-design/` 和 `.../cpp-language/` 下：

- `software-design/round1_solid.md`：OCP、SRP、DIP 构成 Phase 2 抽象的理论依据
- `software-design/round2_creational_structural.md`：Factory、Facade、Adapter 对应本计划的 `BackendRegistry`、`CustomComm*` 入口、`torch_ext/` 桥接
- `software-design/round3_behavioral.md`：Strategy 对应 backend 选择、Command 对应 `prepare/execute/destroy`
- `software-design/round4_modularity_contracts.md`：Parnas 信息隐藏、SOLID 原则、契约设计 — Phase 1/2 的理论根据
- `cpp-language/round1_polymorphism.md`：虚函数 vs C 函数指针 vs type erasure — 决定了 C ABI vtable 的实现形态

## 附录 C：测试与 CI 矩阵（Phase 2 开始执行）

| 层次 | 工具 | 何时跑 | 覆盖什么 |
|---|---|---|---|
| 单元 | `gtest`（C++） | 每次 PR | `BackendRegistry::List/Select`、dtype mapper、desc 合法性校验 |
| ABI 防火墙 | `tests/test_abi_firewall.py` | 每次 PR | shim `.so` 无 `std::` 符号、ext `.so` 不依赖 CANN 内部符号 |
| Meta | pytest + `device="meta"` | 每次 PR | schema / shape 推断，无 NPU 也能跑 |
| CCU 档位 | `tests/ccu_ms/size_boundary_test.py` | 每次 PR（如果 `CUSTOM_COMM_USE_CCU=1` 可用） | LoopGroup 档位 |
| 多 backend 对照 | `tests/backend_equivalence_test.py` | bluezone nightly | 同输入 `ccu` 与 `decomposed` 输出 allclose |
| 性能基准 | `tests/bench/allgather_batch_bench.py` | 手动 / 周度 | 每个 backend 的 GB/s、CPU 开销 |

backend_equivalence_test 是本计划新引入的关键 gate：没有它，Phase 2 的 backend 重构就缺少正确性锚点。实现时用 `seed=42` 固定 HCCL group，跑所有 registered backend，assert `allclose(ref, backend_out, rtol=1e-6)`。

## 评审与回滚

- **Phase 0**：记录 shim / torch_ext 两个 `.so` 的 `nm -D` 符号基线，提交到 `docs/design/symbol-baseline.md`。后续阶段结束时 diff 比对，导出符号超出允许列表（`HcclAllGatherBatch`、`HcclAllGatherBatchBackend*` 等）需要单独 review。
- **每阶段回滚点**：每个 Phase 完成后打 git tag（`refactor/phase-1-complete` 等）。若下一 Phase 的功能回归，可以 revert 到上一个 tag 而不影响 main。
- **ABI 版本号**：头文件 `include/custom_comm/version.h` 定义 `CUSTOM_COMM_ABI_VERSION_MAJOR`（初始 1）。Phase 4 若引入不兼容变更，Major 递增；Phase 2-3 只增加新 struct 字段（必须向后兼容）不算不兼容。
- **回归测试门槛**：Phase 结束时 `pytest tests/ -x` 必须全绿；新增 backend 在 ablation 表格里跑一遍（不同 dtype、不同 rank count、不同 descCount）。
